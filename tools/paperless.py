...
title: Paperless
author: Jonas Leine
funding_url: https://github.com/JLeine/open-webui/blob/main/tools/paperless.py
description: Tool to interact with paperless-ngx documents
version: 1.1.0
license: MIT
"""
...
class PaperlessDocumentLoader(BaseLoader):
    ...
    def __init__(
        self,
        documentTypeName: Optional[str] = "",
        documentTagName: Optional[str] = "",
    ...
    def lazy_load(self) -> Iterator[Document]:  # <-- Does not take any arguments
        ...
        querystring = {
            "document_type__name__icontains": self.documentTypeName,
            "tags__name__icontains": self.documentTagName,
            "created__month": self.created_month,
            "created_year": self.created_year,
            "correspondent__name__icontains": self.correspondent,
        }
...
class PaperlessDocumentLoaderTest(unittest.IsolatedAsyncioTestCase):
    async def assert_document_response(
        self, documentTypeName: str, expected_documents: int
    ):
        ...
        documents = await paperless_tool.get_paperless_documents(
            documentTypeName, "YourTagType", "YourCorrespondent", 2024, 7
        )
