...
description: This function makes charts out of data in the conversation and render it in the chat.
...
class Action:
    class Valves(BaseModel):
        ...
        model_name: str = Field(
            default="gpt-4o-mini", description="Model to build visualization file."
        )
        OPENAI_API_KEY: str = Field(
            default=os.getenv("OPENAI_API_KEY", "api_key"),
            description="key to consume OpenAI interface like LLM for example a litellm key.",
        )
        OPENAI_API_BASE_URL: str = Field(
            default=os.getenv("OPENAI_API_BASE_URL", "base_url"),
